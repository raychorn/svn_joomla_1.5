...
class ps_session {
	...
	function doCookieCheck() {
		global $mm_action_url, $VM_LANG,$mosConfig_live_site;
		$doCheck = mosGetParam( $_REQUEST, 'vmcchk', 0 );
		echo '<span id="ctest" style="visibility:visible"></span>';
		...
		{
		 parse_str($urlArr['query'],$arr2);
		  if(isset($arr2['page'])&& $arr2['page']=='shop.browse')
			$redirect = true;
		}
		*/
		if( $doCheck ) {
            ...
            function fnSendFeedback(url)
            {
                ...
                xmlhttp.onreadystatechange=function() 
                {
                    if (xmlhttp.readyState==4) 
                    {
                         ...
                         {
                            document.getElementById("ctest").innerHTML="<div class='shop_info'><b>Info</b>:Your browser does not accept cookies. If you want to put products into your cart and purchase them you need to enable cookies.<br />";   
                         }
                         else if(xmlhttp.responseText == "2")
                         {
                            fnSendFeedback('<?php print $mosConfig_live_site."/administrator/components/com_virtuemart/classes/serrbiz_vm_cookie_patch.php?step=2"; ?>') 
                         }
                         else if(xmlhttp.responseText == "3")
                         {
                             //alert(getCookie("VMCHECK"));
                             //alert("Anup");                             
                         }
                    }
			    ...
			    fnSendFeedback('<?php print $mosConfig_live_site."/administrator/components/com_virtuemart/classes/serrbiz_vm_cookie_patch.php?step=1"; ?>') 
               //fnSendFeedback('<?php //print $mosConfig_live_site."/req.php?step=2"; ?>')                     
			</script>
		...
		switch ($text) {
			...
			default:
				...
				if (!defined( '_PSHOP_ADMIN' )) {
                    ...
                    //print "<br>Sending: ".$appendix;                    
					$appendix = sefRelToAbs( str_replace( $prep.'&', $prep.'?', $appendix ) );
                    //print "<br>Received: ".$appendix;                    
