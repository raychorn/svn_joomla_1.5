<?php
/**
* @version $Id: moderate_messages.php 63 2007-04-17 21:23:17Z danialt $
* Fireboard Component
* @package Fireboard
...
{
    global $fbConfig;
    echo '<form action="' .JB_LIVEURLREL.'&amp;func=review'. '" name="moderation" method="post">';
?>
    ...
    <table width = "100%" border = 0 cellspacing = 1 cellpadding = 3>
        ...
        foreach ($allMes as $message)
        {
            ...
            echo '<td valign="top">' . smile::smileReplace($message->message, 0, $fbConfig['disemoticons'], $smileyList) . '</td>';
...
function jbApprovePosts($database, $cid)
{
    ...
    $ids = implode(',', $cid);
    $database->setQuery('UPDATE `#__fb_messages` SET `hold` =0 WHERE `id` IN (' . $ids . ')');
