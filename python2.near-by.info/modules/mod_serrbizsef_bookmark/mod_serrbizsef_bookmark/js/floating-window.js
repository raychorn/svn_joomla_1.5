...
************************************************************************************************************/
	...
	function initResizeWindow(e)
	{
		if(document.all)e = event;
		initResizeCounter = 0;
		switchElement(false,document.getElementById('sbz_book_mark_id' + this.id.replace(/[^\d]/g,'')));
	...
	function minimizeWindow(e,inputObj)
	{
		...
		switchElement(false,document.getElementById('sbz_book_mark_id' + numericID));
		var state;	
	...
	{		
        ...
        document.getElementById('windowContent' + divId).innerHTML = document.getElementById('CloseDiv').innerHTML;;
        document.getElementById('windowContent' + divId).innerHTML = (document.getElementById('windowContent' + divId).innerHTML) + document.getElementById('book_detail').innerHTML;
        
		if(urlToExternalFile)addContentFromUrl(urlToExternalFile,divId);	// Add content from external file
        isWindowDisplayed = true;
	}
	
    function fnShowBookContainer(e,position,imgPath)
    {
        ...
        if(position=='bottom')
        {
           tempX = AnchorPosition_getPageOffsetLeft(document.getElementById('bookmark_link'));  
           tempY = document.getElementById('bookimg').height + AnchorPosition_getPageOffsetTop(document.getElementById('bookmark_link'));               
           
        }else if(position=='left')
        {
           tempX = AnchorPosition_getPageOffsetLeft(document.getElementById('bookmark_link')) - winWidth; 
           tempY = AnchorPosition_getPageOffsetTop(document.getElementById('bookmark_link'));                
        }
        else if(position=='right')
        {
           tempX = AnchorPosition_getPageOffsetLeft(document.getElementById('bookmark_link')) + document.getElementById('bookimg').width; 
           tempY = AnchorPosition_getPageOffsetTop(document.getElementById('bookmark_link'));                
        }
        else if(position=='top')
        {
           tempX = AnchorPosition_getPageOffsetLeft(document.getElementById('bookmark_link'));
           tempY = (AnchorPosition_getPageOffsetTop(document.getElementById('bookmark_link')) - winHeight) - document.getElementById('bookimg').height;               
        }
        customFunctionCreateWindow(false,winWidth,winHeight,tempX,tempY)    
    } 
